...
function HomePageNavbar() {
    ...
    const [isSafeMode, setIsSafeMode] = useState(false); // State for Safe mode

    // Check if the Safe mode is saved in localStorage on component mount
    useEffect(() => {
        const savedMode = localStorage.getItem('safeMode');
        if (savedMode === 'true') {
            setIsSafeMode(true);
        } else {
            setIsSafeMode(false);
    ...
    const handleSwitchChange = (checked) => {
        ...
        localStorage.setItem('safeMode', checked); // Store the mode in localStorage
    };
                    ...
                    <div className="collapse navbar-collapse" id="mynavbar">
                        <ul className="navbar-nav navbar_list mx-auto">
                            <li className="nav-item navbar_nav-item">
                                <Link className="nav-link navbar-links-homepage" to="/">
                                    Home
                                </Link>
                            </li>
                            <li className="nav-item navbar_nav-item">
                                <Link className="nav-link navbar-links-homepage" to="/about">
                                    About
                                </Link>
                            </li>
                            <li className="nav-item navbar_nav-item">
                                <Link className="nav-link navbar-links-homepage" to="/services">
                                    Services
                                </Link>
                            </li>
                            <li className="nav-item navbar_nav-item">
                                <Link className="nav-link navbar-links-homepage" to="/contact">
                                    Contact
                                </Link>
                            </li>
                        </ul>
                        <form className="d-flex ms-auto" onSubmit={handleSearch}>
                            <div className="input-group searchbar-grp">
                                ...
                                <button
                                    type="submit"
                                    className="btn searchbar-input-button"
                                    aria-label="Search"
                                >
                        ...
                        {/* Only show the Safe icon if Safe mode is true */}
                        {isSafeMode && (
